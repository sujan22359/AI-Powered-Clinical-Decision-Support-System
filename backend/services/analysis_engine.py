...
class AnalysisEngine:
    ...
    def __init__(self, llm_service: Optional[LLMService] = None, 
        ...
        self.threshold_evaluator = ThresholdEvaluator(self.reference_range_db)
    ...
    def _merge_risk_indicators(
        ...
        Combines risks from both sources, deduplicates if the same parameter
        is detected by both methods, and sorts by severity.
        ...
        Returns:
            Merged and sorted list of risk indicators
        """
        # Start with threshold-based risks (more reliable)
        ...
        for risk in threshold_risks:
            if risk.parameter_name:
                threshold_param_names.add(risk.parameter_name.lower())
        
        # Add AI-detected risks, but skip if same parameter already detected by threshold
        for ai_risk in ai_risks:
            # Check if this AI risk might be a duplicate of a threshold risk
            is_duplicate = False
            finding_lower = ai_risk.finding.lower()
            
            for param_name in threshold_param_names:
                # Simple check: if parameter name appears in AI finding
                if param_name.lower() in finding_lower:
                    is_duplicate = True
                    self.logger.debug(
        ...
        self.logger.info(
            f"Merged risk indicators: {len(threshold_risks)} threshold-based, "
